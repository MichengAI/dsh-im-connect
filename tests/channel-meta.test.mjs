...
test('设置页渠道顺序与视觉稿一致，并包含 Telegram', () => {
  assert.deepEqual(CHANNEL_ORDER, ['dingtalk', 'feishu', 'lark', 'weixin', 'wecom', 'telegram'])
  assert.deepEqual(listChannelMeta().map((item) => item.label), ['钉钉', '飞书', 'Lark', '微信', '企业微信', 'Telegram'])
})
