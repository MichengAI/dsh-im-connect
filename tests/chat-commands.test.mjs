...
function fixture() {
  ...
  const rows = [
    { sessionId: "s1", cwd: "D:\\one", running: false },
    { sessionId: "s2", cwd: "D:\\two", running: false },
  ];
  const services = {
    sessionController: {
            ...
            records: [
              {
                ...
                event: {
                  ...
                  data: {
                    source: { kind: "plugin" },
                    content: [{ type: "text", text: "secret system note" }],
                  },
                },
              },
              {
    ...
    workspaceController: {
      async *follow() {
        yield {
          ...
          value: {
            items: [
              { workspaceId: "w1", path: "D:\\one", sessionIds: ["s1"] },
              { workspaceId: "w2", path: "D:\\two", sessionIds: ["s2"] },
            ],
            archivedSessionIds: [],
          },
        };
    ...
    commands: {
      list(received) {
        ...
        return [{ name: "custom", description: "会话插件命令" }];
      },
      async execute(received, line, images, signal) {
        assert.equal(received, agent);
        calls.push(["command", line, images, signal]);
        return { result: { kind: "success", text: "宿主结果" } };
      },
    },
  };
  const router = {
  ...
  const run = (text, extra = {}) =>
    runner.execute(
      { id: "bot", label: "机器人", status: () => "在线" },
      { chatId: "chat", userId: "u", kind: "dm", text, ...extra },
...
test("Chat 注册命令动态发现、原样执行并返回宿主结果", async () => {
  ...
  f.services.commands.list = () => [
    { name: "installed-later", description: "新安装命令" },
  ];
...
test("图片交给同一命令注册表审批，不绕开图片准入", async () => {
  ...
  assert.deepEqual(f.calls.find((c) => c[0] === "command")[2], [
    { data: "AQI=", mediaType: "image/png", name: "a.png" },
  ]);
  await assert.rejects(
    f.run("/stop", { media: [{ kind: "image" }] }),
    /不接受附件/,
  );
});
...
test("停止、steer、队列使用 Chat 公共入口", async () => {
  const f = fixture();
  await f.run("/stop");
  await f.run("/steer 补充要求");
  assert.deepEqual(f.calls.find((c) => c[0] === "cancel")[1], {
    sessionId: "s1",
  });
  assert.equal(f.calls.find((c) => c[0] === "prompt")[1].mode, "steer");
  assert.match(await f.run("/queue"), /q1/);
  await f.run("/queue edit q1 新要求");
  assert.deepEqual(f.calls.find((c) => c[0] === "queue")[1], {
    sessionId: "s1",
    itemId: "q1",
    action: { kind: "edit", content: [{ type: "text", text: "新要求" }] },
  });
});
...
test("运行中的当前或目标会话不能换绑", async () => {
  ...
  assert.equal(
    f.calls.some((c) => c[0] === "bind"),
    false,
  );
  f.rows[0].running = false;
  f.rows[1].running = true;
...
test("历史只显示用户与助手正文，并关闭订阅", async () => {
  ...
  assert.match(history, /你好/);
  assert.match(history, /回答/);
...
test("推理恢复默认不携带旧强度，模型设置仅修改当前会话", async () => {
  ...
  assert.deepEqual(f.calls.find((c) => c[0] === "model")[1], {
    sessionId: "s1",
    provider: "p",
    model: "m",
  });
  ...
  await f.run("/models");
  await f.run("/model 1 high");
  assert.equal(
    f.calls.filter((c) => c[0] === "model").at(-1)[1].reasoningEffort,
    "high",
  );
});
...
test("查询中取消命令后不继续改模型", async () => {
  ...
  f.services.sessionController.modelCatalog = async () => {
    scope.abort();
    return { default: { provider: "p", model: "m" }, groups: [] };
  };
  await assert.rejects(
    f.runner.execute(
      { id: "bot" },
      { chatId: "chat", userId: "u", text: "/model p/m" },
      scope.signal,
    ),
    { name: "AbortError" },
  );
  assert.equal(
    f.calls.some((call) => call[0] === "model"),
    false,
  );
});

test("真实 Host 命令注册表执行同一 handler 并写入规范命令事件", {
  skip: !process.env.DSH_CHAT_CONTRACT_ROOT,
}, async (t) => {
  const { join, dirname } = await import("node:path");
  const { pathToFileURL } = await import("node:url");
  const { createRequire } = await import("node:module");
  const entry = join(
    dirname(process.env.DSH_CHAT_CONTRACT_ROOT),
    "dsh-commands",
    "lib",
    "index.js",
  );
...
for (const mode of ["resolve", "bind", "abort", "attach"]) {
  test(`fork 创建后 ${mode} 返回新 ID 和补救方法，保留旧绑定`, async () => {
    ...
    f.services.sessionController.fork = async () => {
      ...
      if (mode === "attach")
        throw Object.assign(new Error("挂载失败"), {
          code: "session/workspace-attach-failed",
          details: { sessionId: "forked" },
        });
      return { sessionId: "forked" };
    };
    if (mode === "resolve")
      f.services.sessionController.resolveAgent = async () => {
...
test("fork 成功切换，创建前失败则不虚报已创建", async () => {
  ...
  failed.services.sessionController.fork = async () => {
    throw new Error("没有完整回合");
  };
...
test("命令回复使用宿主语言偏好，切换即时生效且用户内容不翻译", async () => {
  ...
  assert.match(
    ...
    /Restore default: \/reasoning --default/,
  );
  ...
  assert.match(
    await f.run("/steer 中文要求 {0} /model"),
    /中文要求 \{0\} \/model/,
  );
...
test("扩展回复保留错误和反馈披露，目标指引依据结构化状态", async () => {
  const f = fixture();
  f.services.commands.execute = async () => ({
    result: { kind: "success", text: "Shared with maintainer.\n完整反馈披露" },
  });
  assert.match(
    await f.run("/feedback hello"),
    /Shared with maintainer\.\n完整反馈披露/,
  );
  f.services.commands.execute = async () => ({
    result: { kind: "error", text: "  exact error: invalid path\n" },
  });
  assert.match(
    await f.run("/compact"),
...
test("状态使用真实投影；可选数据失败保留可读取字段", async () => {
  ...
  f.services.sessionController.control = async function* () {
...
test("改名回显旧名称，分叉缺回合按错误码提供指引", async () => {
  ...
  f.services.sessionController.fork = async () => {
    throw Object.assign(new Error("technical details"), {
...
test("占用项不推荐，归档查询失败不猜测可用性", async () => {
  ...
  f.router.bind = async () => {
    throw Object.assign(new Error("internal"), { code: "im/session-in-use" });
  };
  f.services.workspaceController.follow = async function* () {
    yield {
      value: {
        items: [{ workspaceId: "w", path: "D:\\two", sessionIds: ["s2"] }],
        archivedSessionIds: [],
      },
    };
  };
  ...
  await assert.rejects(
    f.run("/session s2"),
    /connected to another chat.*\/sessions/,
  );
});
...
test("停止后的附加查询超时仍回复，不误报空闲；未知目标保留条件提示", async () => {
  const f = fixture();
  let querySignal;
  f.services.sessionController.list = (_request, signal) => {
    querySignal = signal;
    return new Promise(() => {});
  };
...
test('工作区成功使用名称并保留路径，模型列表推荐其他模型', async () => {
  const f = fixture()
  f.services.workspaceController.follow = async function* () { yield { value: { items: [{ workspaceId: 'w1', title: '旅行计划', path: 'D:\\one', sessionIds: ['s1'] }], archivedSessionIds: [] } } }
  assert.doesNotMatch(await f.run('/workspaces'), /新建并切换：\/workspace 1/)
  assert.match(await f.run('/workspace w1'), /已在「旅行计划」开启新会话。\n工作区：D:\\one/)
  f.services.sessionController.modelCatalog = async () => ({ default: { provider: 'p', model: 'm' }, groups: [{ id: 'p', models: [{ id: 'm', name: 'Current' }, { id: 'other', name: 'Other' }] }] })
  assert.match(await f.run('/models'), /切换：\/model 2/)
  f.services.settings = { get: () => ({ preference: 'en' }) }
  assert.match(await f.run('/models'), /Switch: \/model 2/)
  assert.match(await f.run('/workspace w1'), /Started a new session in 旅行计划/)
