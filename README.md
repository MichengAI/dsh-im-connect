...
命令请单独发送为文字，图片说明维持普通消息语义。`/help` 动态发现当前会话的 Chat 注册命令并复用宿主执行逻辑。列表序号按用户保存，15 分钟有效；切换前须结束运行任务和待处理交互。队列支持 `/queue remove|steer|edit <消息ID> [新内容]`，归档会话需先在 Chat 恢复。

命令回复支持中英文，并跟随网页中明确保存的语言选择；未指定语言或语言服务不可用时使用中文。回复提供相关命令和可复制用法，动态名称、路径、用户内容与宿主扩展结果保持原文。
