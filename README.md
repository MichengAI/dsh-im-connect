...
Telegram、飞书/Lark 使用原生按钮；企业微信在按钮数量符合平台限制时使用卡片，其他情况及钉钉、QQ、微信使用文字序号。平台明确拒绝或超出卡片容量时退回完整文字；网络异常导致发送结果未知时仅提示确认，避免重复发送。菜单按账号、聊天、操作者和会话隔离，15 分钟或重启后失效；执行时重新检查权限，旧按钮不能重复执行。
