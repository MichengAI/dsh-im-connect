...
- 当前源码支持 DSH `0.1.2-rc.1`、`0.1.5-rc.1`，推荐后者；开发依赖固定为 `0.1.5-rc.1`。`0.1.0-rc.8`、`0.1.1-rc.2` 缺少本插件需要的认证与会话控制接口，请先升级 DSH。
...
- 权限与导出：`/permission` 设置在恢复会话时保留；`/export` 在网页 Chat 中导出，IM 暂不回传 ZIP 文件。
