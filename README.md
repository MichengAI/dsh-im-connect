...
- 主动投递：在账号设置中命名并开启投递，保存接收目标后，通过 `/im-send` 或 Agent 工具发送文字。需要 DSH `0.1.2-rc.1` 及以上兼容版本，微信受会话限制。见 [使用说明](docs/02-产品与业务/05-主动投递.md) 和 [接口文档](docs/03-技术架构/05-主动投递接口.md)。
