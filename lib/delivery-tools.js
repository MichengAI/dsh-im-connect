...
export function assertDeliveryCaller(exec) {
...
async function invoke(exec, action) {
    try {
...
export function registerDeliveryTools(ctx, manager) {
    const disposers = [];
    const requireAccount = (accountId) => {
        if (!manager.deliveryAccounts().some(account => account.accountId === accountId))
            throw new DeliveryError('delivery-disabled', '账号不存在或未允许主动投递', 403);
    };
    try {
