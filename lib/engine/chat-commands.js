...
export class ChatCommands {
    ...
    service(name) {
        ...
        if (!value)
            throw new Error(`当前 Host 未提供 ${name}，请升级 DSH。`);
        return value;
    }
    async call(service, method, ...args) {
        ...
        if (typeof target[method] !== 'function')
            throw new Error(`当前 Host 不支持 ${method}。`);
    ...
    async agent(sessionId) {
        ...
        if (!result.agent)
            throw new Error(result.error?.message || '无法打开会话。');
        return result.agent;
    }
    ...
    resolve(key, value) {
        ...
        if (!selected)
            throw new Error('序号无效或已过期，请重新获取列表。');
        return selected;
    }
    ...
    async run(channel, msg, signal) {
        ...
        if (!match)
            throw new Error('命令格式无效，请发送 /help。');
        ...
        if (msg.media?.length && ['help', 'new', 'clear', 'sessions', 'sessionlist', 'session', 'workspaces', 'workspacelist', 'workspace', 'models', 'status', 'current', 'stop', 'steer', 'rename', 'fork', 'model', 'reasoning', 'reasonings', 'reasoninglist', 'history', 'queue'].includes(command))
            throw new Error('该控制命令不接受附件，请单独发送。');
        const current = this.router.lookup(channel.id, kind, msg.chatId);
        const requireCurrent = () => { if (!current)
            throw new Error('当前没有会话，请先发送消息或 /new。'); return current.sessionId; };
        if (command === 'export')
            return EXPORT_HINT;
        if (command === 'help') {
            let commands = [];
        ...
        if (command === 'sessions' || command === 'sessionlist') {
            ...
            if (!Number.isInteger(page) || page < 1)
                throw new Error('用法：/sessions [页码]');
        ...
        if (command === 'session') {
            if (!input)
                return current ? `当前会话：${current.sessionId}` : '当前没有会话。';
        ...
        if (command === 'workspaces' || command === 'workspacelist' || command === 'workspace') {
            ...
            if (!workspace)
                throw new Error('请使用 /workspaces 中的工作区。');
            await this.idle(current?.sessionId, signal);
        ...
        if (command === 'queue') {
            ...
            if (!action || (action[1] === 'edit' && !action[3]) || (action[1] !== 'edit' && action[3]))
                throw new Error('用法：/queue remove|steer|edit <消息ID> [新内容]');
        ...
        if (command === 'fork') {
            ...
            if (!workspace)
                throw new Error('当前会话工作区不可用，无法分叉。');
            let createdId;
            try {
                signal.throwIfAborted();
                ...
                if (typeof controller.fork !== 'function')
                    throw new Error('当前 Host 不支持 fork。');
        ...
        if (command === 'model' || command === 'reasoning' || command === 'reasonings' || command === 'reasoninglist') {
            ...
            if (command === 'model') {
                if (!input)
                    return `${selected.provider}/${selected.model}；推理：${selected.reasoningEffort || '默认'}`;
                const [choice, reasoningEffort] = input.split(/\s+/);
                const id = this.resolve(scope + ':models', choice);
                const slash = id.indexOf('/');
                if (slash < 1)
                    throw new Error('用法：/model <序号或provider/model> [推理等级]');
        ...
        const images = (msg.media || []).map(media => {
            if (media.kind !== 'image' || !media.data || !media.mediaType)
                throw new Error('Chat 命令只支持文字和图片附件。');
