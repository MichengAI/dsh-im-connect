...
export class ChatCommands {
    ...
    async run(channel, msg, signal, navigation) {
        ...
        if (msg.media?.length && ['help', 'new', 'clear', 'sessions', 'sessionlist', 'session', 'workspaces', 'workspacelist', 'workspace', 'models', 'status', 'current', 'stop', 'steer', 'rename', 'fork', 'model', 'reasoning', 'reasonings', 'reasoninglist', 'history', 'queue', 'export'].includes(command))
        ...
        if (command === 'menu' || command === 'm') {
            ...
            if (extra.length || !['', 'sessions', 'workspaces', 'models'].includes(section) || !Number.isSafeInteger(page) || page < 1)
                throw new Error(replyText('用法：/menu [sessions|workspaces|models] [页码]'));
            let choices = [];
