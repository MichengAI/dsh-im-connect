...
export const chatControlHelp = () => [
    replyText("IM 助理已连接 DeepSeek Harness。直接发送文字即可开始任务。"), replyText("例如：帮我整理今天的待办。查看图片或文件支持范围，请在网页打开 IM 助理说明。"), '',
    replyText("会话与工作区"),
...
export class ChatCommands {
    ...
    async run(channel, msg, signal) {
        ...
        if (command === 'new' || command === 'clear') {
            ...
            return [replyText("已开启新的频道会话"), await this.newDetails(next, signal), replyText("旧会话已保留，直接发送消息即可开始。")].filter(Boolean).join('\n') + related(replyText("查看旧会话：/sessions"), replyText("查看当前配置：/status"));
        }
