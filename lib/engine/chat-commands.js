...
export class ChatCommands {
    ...
    resolve(key, value) {
        ...
        if (!selected)
            throw new Error(replyText('序号无效或已过期，尚未切换。请发送 /{0} 获取新列表。', key.endsWith(':presets') ? 'presets' : key.endsWith(':models') ? 'models' : key.endsWith(':workspaces') ? 'workspaces' : 'sessions'));
        return selected;
    }
    ...
    async run(channel, msg, signal, navigation) {
        ...
        if (command === 'menu' || command === 'm') {
            ...
            if (extra.length || !['', 'sessions', 'workspaces', 'models', 'presets'].includes(section) || !Number.isSafeInteger(page) || page < 1)
                throw new Error(replyText('用法：/menu [sessions|workspaces|models|presets] [页码]'));
            let choices = [];
