...
export class ChatCommands {
    ...
    choices = new Map();
    reasoningChoices = new Map();
    constructor(host, router, pending, onSession = () => { }, showChoices) {
        this.host = host;
        this.router = router;
    ...
    clear() { this.choices.clear(); this.reasoningChoices.clear(); }
    service(name) {
    ...
    async presets() {
        return (await this.call('agentPresets', 'remoteExportList')).presets;
    }
    ...
    reasoningChoice(scope, sessionId, selected, effort) {
        if (this.reasoningChoices.size >= 512)
            this.reasoningChoices.delete(this.reasoningChoices.keys().next().value);
        ...
        return withReplyLocale(this.host, () => this.lifetime.run(signal, async () => {
            try {
                ...
                if (channel.sendChoices && this.showChoices && !signal.aborted) {
                    try {
                        return await this.showChoices(channel, msg, text, choices, current?.sessionId, false);
                    }
                    catch {
                        return fallback;
                    }
    ...
    async optional(read, parent) {
        ...
        try {
            return await Promise.race([
                ...
                new Promise((resolve) => { timer = setTimeout(() => { scope.abort(); resolve(undefined); }, 800); }),
            ]);
        }
        finally {
            clearTimeout(timer);
    ...
    async newDetails(binding, signal) {
        ...
        const [snapshot, rows] = await Promise.all([
            ...
            cwd ? undefined : this.optional((sig) => this.call('sessionController', 'list', {}, sig), signal),
        ]);
    ...
    async run(channel, msg, signal, navigation) {
        ...
        if (msg.media?.length && ['preset', 'presets', 'presetlist', 'help', 'new', 'clear', 'sessions', 'sessionlist', 'session', 'workspaces', 'workspacelist', 'workspace', 'models', 'status', 'current', 'stop', 'steer', 'rename', 'fork', 'model', 'reasoning', 'reasonings', 'reasoninglist', 'history', 'queue', 'export'].includes(command))
            throw new Error(replyText('该控制命令不接受附件，请单独发送。'));
        const current = this.router.lookup(channel.id, kind, msg.chatId);
        const requireCurrent = () => { if (!current)
            throw new Error(replyText('当前没有会话，请先发送消息或 /new。')); return current.sessionId; };
        if (input && ['presets', 'presetlist', 'help', 'new', 'clear', 'workspaces', 'workspacelist', 'models', 'status', 'current', 'stop', 'fork', 'history', 'reasonings', 'reasoninglist', 'export'].includes(command))
        ...
        if (command === 'menu' || command === 'm') {
            ...
            if (extra.length || !['', 'sessions', 'workspaces', 'models', 'presets', 'reasoning'].includes(section) || !Number.isSafeInteger(page) || page < 1)
                throw new Error(replyText('用法：/menu [sessions|workspaces|models|presets|reasoning] [页码]'));
            let choices = [];
            ...
            if (section === 'sessions') {
                ...
                if (efforts.length)
                    choices.push({ label: replyText('恢复默认推理'), value: `/reasoning ${this.reasoningChoice(scope, sessionId, selected)}` });
            }
            ...
            }
            {
                ...
                if (page > pages)
                    throw new Error(replyText('没有第 {0} 页，共 {1} 页。', page, pages));
                ...
                if (section === 'reasoning')
                    choices = choices.map((choice, i) => ({ ...choice, displayValue: `/reasoning ${i + 1}` }));
                if (section)
                    this.remember(`${scope}:${section}`, choices.map(choice => choice.value.slice(choice.value.indexOf(' ') + 1)));
                const pageCommand = section ? `/menu ${section}` : '/menu';
                if (page > 1)
                    choices.push({ label: replyText('上一页'), value: `${pageCommand} ${page - 1}` });
                if (page < pages)
                    choices.push({ label: replyText('下一页'), value: `${pageCommand} ${page + 1}` });
        ...
        if (command === 'export') {
            const sessionId = requireCurrent();
            return await exportSession(this.host, channel, msg.chatId, sessionId, signal, () => this.router.lookup(channel.id, kind, msg.chatId)?.sessionId === sessionId);
        }
        if (command === 'help') {
            let commands = [];
        ...
        if (command === 'sessions' || command === 'sessionlist') {
            ...
            if (!rows.length)
                return replyText('暂无可接续会话。') + related(replyText('直接发送消息，或 /new 开始聊天'));
            const pages = Math.ceil(rows.length / 10);
            if (page > pages)
                return replyText('没有第 {0} 页，共 {1} 页。', page, pages) + related(replyText('返回列表：/sessions {0}', pages));
        ...
        if (command === 'session') {
            ...
            if (workspace.items.filter((item) => item.sessionIds.includes(id)).length !== 1)
        ...
        if (command === 'workspaces' || command === 'workspacelist' || command === 'workspace') {
            ...
            if (command !== 'workspace' || !input) {
                ...
                return items.length ? replyText('工作区列表 · 共 {0} 个\n\n', items.length) + items.map((item, i) => `${i + 1}. ${item.title ? oneLine(item.title) : oneLine(item.path)}${item.title ? `\n${item.path}` : ''}${current && item.sessionIds.includes(current.sessionId) ? replyText('〔当前〕') : ''}`).join('\n') + related(alternative >= 0
                    ...
                    : replyText('当前只有这个工作区。直接发消息继续，或 /new 开启新会话。'), replyText('也可使用已列出的绝对路径；旧会话保留，账号默认目录不变。'), replyText('别名：/workspacelist；序号 15 分钟内有效。')) : replyText('还没有可用工作区。请先在网页添加，再发送 /workspaces。');
            }
            ...
            return replyText('已在「{0}」开启新会话。\n工作区：{1}\n旧会话已保留，可以直接发送任务。', oneLine(workspace.title || workspace.path), workspace.path) + related(replyText('查看配置：/status'), replyText('切回旧会话：/sessions'));
        }
        if (command === 'models') {
            const catalog = await this.call('sessionController', 'modelCatalog');
            ...
            if (current) {
                try {
                    selected = (await this.snapshot(current.sessionId, signal)).projections?.values?.modelSelection?.next;
                }
                catch {
                    signal.throwIfAborted();
                }
            ...
            const alternative = selected
                ? models.findIndex((model) => model.id !== `${selected.provider}/${selected.model}`)
                : -1;
        ...
        if (command === 'presets' || command === 'presetlist' || command === 'preset') {
            ...
            if (!preset || preset.broken)
                throw new Error(replyText('预设不存在或不可用，请发送 /presets 重新选择。'));
            ...
            if (matches.length !== 1)
                throw new Error(replyText('无法确定当前工作区，请先用 /workspace 选择工作区。'));
            const workspace = matches[0];
            let createdId;
            try {
                const controller = this.service('sessionController');
                if (typeof controller.create !== 'function')
                    throw new Error(replyText('当前 Host 不支持 {0}。', 'create'));
                signal.throwIfAborted();
                const result = await controller.create({ workspaceId: workspace.workspaceId, ...(followDefault ? {} : { agentPreset: id }) });
                ...
                const partial = error;
                if (!createdId && partial.code === 'session/workspace-attach-failed')
        ...
        if (command === 'queue') {
            if (!input) {
                ...
                for await (const frame of stream) {
                    ...
                    if (!items.length)
                        return replyText('当前没有排队消息。') + related(replyText('直接发消息继续'), replyText('查看状态：/status'));
            ...
            if (!action || (action[1] === 'edit' && !action[3]) || (action[1] !== 'edit' && action[3]))
        ...
        if (command === 'status' || command === 'current') {
            ...
            if (values && 'goal' in values) {
                const phases = { active: replyText('活跃'), paused: replyText('已暂停'), blocked: replyText('受阻'), complete: replyText('已完成') };
            ...
            if (phase === 'active' || phase === 'paused' || phase === 'blocked') {
                ...
                if (registered?.some((item) => item.name === 'goal'))
                    actions.push(phase === 'active'
                        ? { label: replyText('暂停目标'), value: '/goal pause' }
                        : { label: replyText('恢复目标'), value: '/goal resume' });
            }
            if (row?.running === true)
                actions.push({ label: replyText('停止任务'), value: '/stop' });
            if (queueCount)
                actions.push({ label: replyText('查看队列'), value: '/queue' });
            if (!actions.length)
                actions.push({ label: replyText('刷新状态'), value: '/status' }, { label: replyText('最近记录'), value: '/history' });
            ...
            ].join('\n') + related(...(row?.running === true ? [replyText('补充要求：/steer 补充内容')] : []), ...actions.slice(0, 3).map(action => replyText('{0}：{1}', action.label, action.value)));
        }
        if (command === 'stop') {
            try {
                ...
                const state = await this.optional((sig) => this.call('sessionController', 'list', {}, sig), signal);
                if (state?.items.find(row => row.sessionId === sessionId)?.running !== false)
                    throw new Error(replyText('暂时无法确认当前会话的运行状态，未能请求停止。请用 /status 查看，或到网页检查会话。'));
        ...
        if (command === 'fork') {
            ...
            try {
                ...
                if (!createdId && attachFailed && typeof partial.details?.sessionId === 'string')
                    createdId = partial.details.sessionId;
        ...
        if (command === 'model' || command === 'reasoning' || command === 'reasonings' || command === 'reasoninglist') {
            ...
            if (command === 'model') {
                ...
                const result = await this.call('sessionController', 'selectModel', { sessionId, provider: id.slice(0, slash), model: id.slice(slash + 1), ...(reasoningEffort ? { reasoningEffort } : {}) });
            ...
            if (resolved.startsWith('--choice ')) {
                ...
                if (!choice || choice.scope !== scope || choice.sessionId !== sessionId || choice.provider !== selected.provider || choice.model !== selected.model || Date.now() - choice.time >= 15 * 60_000)
                    throw new Error(replyText('选择已过期，或会话、模型已变化。请发送 /reasoning 重新选择。'));
                effort = choice.effort ?? '--default';
                this.reasoningChoices.delete(token);
            }
            if (effort !== '--default' && !efforts.some(item => item.id === effort))
