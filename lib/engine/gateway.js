...
export class ImEngine {
        ...
        if (typeof on === 'function') {
            for (const kind of ['inserted', 'claimed', 'discarded']) {
                this.disposeEvents.push(on(`agent/inbox/${kind}`, (...args) => {
                    this.progress.inbox(kind, args[0]);
                }, { global: true }));
            }
    ...
    async notifyCompletion(result) {
        ...
        await withReplyLocale(this.ctx, async () => {
            ...
            if (!choices.length) {
                await this.deliver(channel, message.chatId, text);
                return;
            }
            const fallback = await this.choices.show(channel, message, text, choices, result.sessionId, valid, replyText('点击按钮或发送对应命令；按钮 15 分钟内有效，普通文字继续聊天。'), false);
            if (fallback && valid())
                await this.deliver(channel, message.chatId, fallback);
        });
    }
