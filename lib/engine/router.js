...
export class SessionRouter {
    ...
    setTitle(sessionId, title, source) {
        ...
        if (!rec)
            return false;
        if (!title.trim() || source === 'message' && rec.titleSource || source === 'host' && rec.titleSource === 'user')
            return false;
    ...
    syncStoredTitle(sessionId, agent) {
        ...
        if (typeof latest?.title === 'string')
            this.setTitle(sessionId, latest.title, latest.source?.kind === 'user' ? 'user' : 'host');
    }
