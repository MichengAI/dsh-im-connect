...
export class SessionRouter {
    ...
    isBoundElsewhere(sessionId, channelId, kind, chatId) {
        const key = sessionKeyOf(channelId, kind, chatId);
    ...
    async bind(channelId, kind, chatId, sessionId, title, agent, cwd, signal) {
        await this.channelOperations.run(channelId, async () => {
            const key = sessionKeyOf(channelId, kind, chatId);
            signal?.throwIfAborted();
            const other = this.isBoundElsewhere(sessionId, channelId, kind, chatId);
            if (other)
                throw Object.assign(new Error('该会话已关联其他聊天，不能重复绑定。'), { code: 'im/session-in-use' });
            const old = this.live.get(key);
    ...
    async cleanupMissing(sessionId) {
        ...
        return this.channelOperations.run(rec.channel, async () => {
            ...
            if (!persistence?.list)
                return false;
            try {
                if ((await persistence.list()).map(storedHeader).some(header => header.id === sessionId))
                    return false;
            }
            catch {
                return false;
    ...
    async attachMappedSessions(active = () => true) {
    ...
    presetSetup(channelId, savedPreset, restoring = false) {
        const ctx = this.ctx;
        const config = this.resolveConfig(channelId);
        ...
        return async (agentCtx, explicitAgent) => {
            ...
            const agent = explicitAgent ?? agentCtx.agent;
            const session = agent?.session;
            const events = session?.snapshotEvents?.() ?? session?.events;
