...
export function recoverTurn(events, requestId) {
    ...
    for (const event of events) {
        ...
        if (!matched)
            continue;
        if (event.type === 'assistant/message' && data?.turn === target && !data?.interrupted && !data?.message?.content?.some((p) => p.type === 'tool-call')) {
...
export class DeferredDelivery {
    ...
    begin(id, sessionId, channelId, message) {
        ...
        while (this.entries.length >= 1000 && this.entries.some(e => e.status === 'sent'))
            this.entries.splice(this.entries.findIndex(e => e.status === 'sent'), 1);
    ...
    reject(id) { this.active.delete(id); }
    patch(id, update) {
    ...
    block(channelId) {
        for (const e of this.entries)
            if (e.channelId === channelId && e.status !== 'sent')
    ...
    async recover(id, read, valid, send, chunks, explicit = false) {
        ...
        return this.queue.run(sessionId, async () => {
            const entry = this.entries.find(e => e.id === id);
            if (!entry || this.active.has(id))
                return;
            if (explicit && ['unknown', 'blocked'].includes(entry.status))
                this.patch(id, { status: entry.text ? 'ready' : 'waiting' });
            ...
            if (!entry.text) {
                ...
                if (!result || !['waiting', 'ready'].includes(entry.status))
                    return;
