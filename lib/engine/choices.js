...
export class ChoiceStore {
    ...
    async show(channel, msg, text, choices, session, valid = () => true, hint = replyText('点击选项或回复序号；15 分钟内有效，普通文字退出菜单。')) {
        ...
        if (channel.sendChoices) {
            try {
                await channel.sendChoices(msg, body, choices.map((choice, i) => ({ label: choice.label, token: `${token}:${i}` })));
                return '';
            }
