...
export function createWeixinChannel(config, log, stateDir) {
    ...
    async function request(path, body, timeoutMs, tolerateRet1 = false, signal) {
        ...
        if (ret !== 0 || errcode !== 0) {
            if (path === '/ilink/bot/sendmessage')
                throw new DeliveryError('platform-rejected', `微信拒绝发送（ret=${ret}, errcode=${errcode}），可能受会话或平台限制，请检查目标并由对方重新发消息后再尝试`);
    ...
    return {
        ...
        async sendProactive(route, text, signal) {
            ...
            if (!botToken)
                throw new DeliveryError('offline', '微信账号未登录', 503);
            if (!state.contextTokens[route.nativeId])
                throw new DeliveryError('context-required', '微信目标缺少会话上下文，请先让对方发一条消息');
