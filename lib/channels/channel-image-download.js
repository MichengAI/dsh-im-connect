...
export function channelImageFailureReason(error) {
    const value = error;
    const message = typeof value?.message === 'string' ? value.message : '';
    const rawCode = value?.code ?? value?.cause?.code;
    const code = typeof rawCode === 'string' ? rawCode : undefined;
    if (['图片地址不符合安全要求', '不安全的图片地址'].includes(message) || code === 'ERR_INVALID_URL')
        return '下载地址被安全校验拦截';
    const http = /^图片下载 HTTP (\d{3})$/.exec(message);
    if (http)
        return `下载服务器返回 HTTP ${http[1]}`;
    if (/图片.*(大小|过大|数量)/.test(message))
        return '图片数量或大小超过接收限制';
    if (message === '图片下载超时' || ['TimeoutError', 'AbortError'].includes(value?.name ?? '') || code === 'ETIMEDOUT')
        return '下载超时或已取消';
    if (message === '图片解密失败' || code?.startsWith('ERR_OSSL_'))
        return '图片解密失败';
    if (message === '图片缺少下载地址或解密密钥')
        return '图片缺少下载地址或解密信息';
    if (message.startsWith('不支持的图片格式'))
        return '不支持的图片格式';
    if (message === '图片为空')
        return '服务器返回了空图片';
    if (code === 'ENOTFOUND' || code === 'EAI_AGAIN')
        return 'DNS 解析失败';
    if (code?.startsWith('ERR_TLS_') || code?.startsWith('ERR_SSL_') || code === 'CERT_HAS_EXPIRED')
        return 'TLS 连接校验失败';
    return '网络连接或图片处理失败';
}
/** Diagnostic host only: never emit a signed path/query, userinfo, or AES key. */
export function channelImageDownloadHost(raw) {
    try {
        const value = raw ?? '';
...
export function imageMedia(data) {
    if (!data.length || data.length > MAX_CHANNEL_IMAGE_BYTES)
        throw new Error('图片超过大小限制或为空');
    let mediaType;
    if (data.subarray(0, 8).equals(Buffer.from('89504e470d0a1a0a', 'hex')))
        mediaType = 'image/png';
    else if (data.subarray(0, 3).equals(Buffer.from('ffd8ff', 'hex')))
        mediaType = 'image/jpeg';
    else if (/^GIF8[79]a$/.test(data.subarray(0, 6).toString('ascii')))
        mediaType = 'image/gif';
    else if (data.subarray(0, 4).toString() === 'RIFF' && data.subarray(8, 12).toString() === 'WEBP')
        mediaType = 'image/webp';
    if (!mediaType)
        throw new Error('不支持的图片格式，请发送 PNG、JPEG、GIF 或 WebP');
 ...
 */
export async function requestChannelBytes(rawUrl, options = {}) {
        ...
        const req = https.request(url, {
            ...
            lookup: (hostname, _options, callback) => {
                dns.lookup(hostname, { all: true, family: 4 }, (error, addresses) => {
                    ...
                    if (!addresses.length || addresses.some(item => item.family !== 4
                        || (blocked.check(item.address) && !(proxyCompatibleHosts.has(hostname) && proxyFakeIps.check(item.address))))) {
            ...
            try {
                if (response.statusCode !== 200)
                    throw new Error(`图片下载 HTTP ${response.statusCode}`);
...
//# sourceMappingURL=channel-image-download.js.map