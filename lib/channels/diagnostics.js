export class DiagnosticError extends Error {
    ...
    httpStatus;
    platformCode;
    constructor(reason, httpStatus, platformCode) {
        super(reason);
        this.reason = reason;
...
export async function probe(id, signal, run) {
...
export async function diagnosticJson(url, signal, body, headers) {
    ...
    if (!response.body)
        throw new DiagnosticError('invalid-response');
...
export function requireDiagnostic(value) {
    if (!value)
        throw new DiagnosticError('invalid-response');
}
export function platformResult(code, authCodes = []) {
    ...
    if (!Number.isFinite(numeric))
        throw new DiagnosticError('invalid-response');
    if (numeric !== 0)
        throw new DiagnosticError(authCodes.includes(numeric) ? 'auth' : numeric === 429 ? 'rate-limit' : 'rejected', undefined, numeric);
}
