...
export function parseDingtalkCardAction(raw) {
    try {
        const data = JSON.parse(raw);
        if (data.type !== 'actionCallback' || (data.userIdType !== undefined && data.userIdType !== 1))
            return;
        const content = typeof data.content === 'string' ? JSON.parse(data.content) : data.content;
        const ids = content?.cardPrivateData?.actionIds;
        if (typeof data.outTrackId !== 'string' || typeof data.userId !== 'string' || !data.userId || !Array.isArray(ids) || ids.length !== 1 || typeof ids[0] !== 'string')
            return;
        ...
        async start() {
            ...
            try {
                ...
                client.registerCallbackListener(sdk.TOPIC_CARD, (res) => {
                    ...
                    if (!action || !entry || entry.expires <= Date.now() || entry.message.userId !== action.userId || !entry.tokens.has(action.token))
                        return;
