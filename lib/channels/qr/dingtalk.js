...
export function parseDingtalkInit(body) {
    const record = asRecord(body);
    if (Number(record.errcode ?? 0) !== 0)
        throw new Error('钉钉注册初始化被拒绝');
    const nonce = cleanString(record.nonce);
    if (!nonce)
        throw new Error('钉钉注册初始化没有返回 nonce');
    return nonce;
}
export function parseDingtalkBegin(body, now = Date.now()) {
    const record = asRecord(body);
    if (Number(record.errcode ?? 0) !== 0)
        throw new Error('钉钉注册开始被拒绝');
    const deviceCode = cleanString(record.device_code);
    const verificationUrl = cleanString(record.verification_uri_complete);
    if (!deviceCode || !verificationUrl)
        throw new Error('钉钉注册没有返回完整二维码');
...
export function parseDingtalkPoll(body) {
    const record = asRecord(body);
    if (Number(record.errcode ?? 0) !== 0)
        throw new Error('钉钉扫码查询被拒绝');
    ...
    if (raw === 'SUCCESS') {
        ...
        if (!clientId || !clientSecret)
            throw new Error('钉钉扫码结果缺少 Client 凭据');
        return { status: 'success', credentials: { clientId, clientSecret } };
    }
    if (raw === 'EXPIRED')
        return { status: 'expired', error: '二维码已过期' };
    if (raw === 'FAIL')
        return { status: 'failed', error: cleanString(record.fail_reason) ?? '扫码未完成' };
...
export async function beginDingtalkQr(signal) {
