...
export function parseFeishuPoll(body, currentBase) {
    ...
    if (err === 'slow_down')
        return { status: 'waiting', baseUrl, pollIntervalMs: 10_000 };
    if (err === 'access_denied')
        return { status: 'failed', error: '用户拒绝授权', baseUrl };
    if (err === 'expired_token')
        return { status: 'expired', error: '二维码已过期', baseUrl };
    if (err)
        return { status: 'failed', error: err, baseUrl };
    return { status: 'waiting', baseUrl };
}
async function call(baseUrl, action, params, signal) {
...
export async function beginFeishuQr(domain, signal) {
