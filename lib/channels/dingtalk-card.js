...
export class DingtalkCardClient {
    ...
    async createChoices(id, target, text, buttons, signal) {
        const data = (status) => ({ cardParamMap: {
        ...
        try {
            await this.request('v1.0/card/instances/deliver', deliverBody(id, target, this.clientId), headers, 'POST', signal);
        }
        catch (error) {
            throw choiceSendError(error);
        }
        return { close: async (status) => {
                await this.request('v1.0/card/instances', { outTrackId: id, cardData: data(status) }, { 'x-acs-dingtalk-access-token': await this.accessToken() }, 'PUT', signal);
            } };
    }
    ...
    async request(path, body, headers, method = 'POST', signal) {
    ...
    return {
        async update(text) {
            ...
            timer = setTimeout(() => {
                ...
                const next = pending;
                if (next)
                    inflight = inflight.then(() => flush(next)).catch((error) => {
