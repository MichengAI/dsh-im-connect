...
function isExpiredCallback(error) {
    return Number(error?.errcode) === 846605;
}
...
export function createWecomChannel(config, log, dependencies = {}) {
    ...
    return {
        ...
        async start() {
            ...
            client.on('authenticated', () => { if (generation === startedGeneration)
                statusText = '长连接已建立'; });
            client.on('disconnected', () => { if (generation === startedGeneration)
                statusText = '已断开'; });
            client.on('reconnecting', () => { if (generation === startedGeneration)
                statusText = '重连中'; });
            ...
            client.on('event.template_card_event', (frame) => {
                ...
                // 按钮事件仅用于更新原卡片，不能作为普通消息 replyStream 的回调帧。
        ...
        async diagnose(signal) {
            return [await probe('heartbeat', signal, async () => {
                    if (!client?.isConnected)
                        throw new DiagnosticError('not-connected');
                    if (!client.reply)
                    ...
                    try {
                        ...
                        if (error instanceof Error && /timeout/i.test(error.message))
                            throw new DiagnosticError('timeout');
                        throw error;
                    }
                })];
        },
