...
export function createTelegramChannel(config, log) {
    ...
    return {
        ...
        async sendChoices(message, text, buttons) {
            const sent = await api('sendMessage', { chat_id: message.chatId, text, reply_markup: { inline_keyboard: buttons.map(button => [{ text: button.label.slice(0, 60), callback_data: button.token }]) } });
