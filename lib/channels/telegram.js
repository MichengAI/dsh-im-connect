...
export function createTelegramChannel(config, log) {
    ...
    return {
        ...
        async sendProactive(route, text, signal) {
            ...
            if (data.ok === false)
                throw new DeliveryError('platform-rejected', `Telegram 拒绝发送（${data.error_code ?? response.status}），请检查目标、机器人权限或频率限制`);
            if (!response.ok || data.result?.message_id === undefined)
                throw new Error('Telegram 未返回有效发送回执');
