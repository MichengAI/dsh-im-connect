...
export function createQqChannel(config, log) {
    ...
    async function sendText(chatId, text) {
        ...
        const body = { content: text, msg_type: 0, msg_seq: nextSeq };
        if (target?.lastMsgId)
            body.msg_id = target.lastMsgId;
    ...
    return {
        ...
        async sendProactive(route, text, signal) {
            ...
            try {
                ...
                throw new DeliveryError('authentication-failed', 'QQ 账号鉴权失败，请重新连接', 503);
            }
            ...
            if (data.code || (response.status >= 400 && response.status < 500))
                throw new DeliveryError('platform-rejected', `QQ 拒绝主动发送（${data.code ?? response.status}），请检查主动消息许可、目标和频率限制`);
            if (!response.ok || !data.id)
                throw new Error('QQ 未返回有效发送回执');
