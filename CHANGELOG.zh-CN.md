...
- 修复 #12：`/new`、`/clear` 和工作区切换保留旧频道会话；打开、改名和归档历史会话不改变当前 IM 聊天绑定。
- 启动时从宿主持久化日志补回可识别 IM ID 的历史索引，保留归档状态；恢复失败不创建空会话，持久化状态未知时不删除索引。
- 同步宿主自动与手动标题，保护手动名称和来源未知的旧名称；新会话以首条消息或附件名兜底，安全截断 Unicode 文本。
- 归档或删除失败不再移除频道入口；已确认删除的历史日志不再因残留归档标记保留无效索引。同步中英文 README 与聊天帮助。
- 本机启用真实宿主包合同测试后 416 项全部通过（0 跳过）；真实 DSH 界面、手机渠道和归档管理器端到端联调仍未验证。升级后请重启 DSH 并刷新页面。
