...
export async function exportSession(host: { get(name: string): unknown }, channel: ChannelAdapter,
  chatId: string, sessionId: string, parent: AbortSignal, stillCurrent: () => boolean): Promise<string> {
  const signal = timeoutSignal(120_000, parent)
  const fallback = () => replyText('会话未能导出或发送。请在网页 Chat 中打开这条会话，执行 /export 下载 ZIP。')
  try {
    signal.throwIfAborted()
    const connection = host.get('connection') as { createSharedFetchHandler?: (channel: '/api') => { fetch(request: Request): Promise<Response> } } | undefined
    if (!channel.sendFile || !connection?.createSharedFetchHandler) throw new Error('export-unavailable')
    if (!stillCurrent()) throw new Error('binding-changed')
    ...
    let size = 0
    try {
      if (!response.ok || !response.headers.get('content-type')?.toLowerCase().startsWith('application/zip') || !reader) throw new Error('export-response')
      for (;;) {
        const part = await fileOperation(reader.read(), signal)
        if (part.done) { complete = true; break }
        size += part.value.byteLength
        // 渠道接口接收完整字节；流式限额避免会话附件耗尽插件内存。
        if (size > 32 * 1024 * 1024) throw new Error('export-too-large')
    ...
    if (!size) throw new Error('export-empty')
    signal.throwIfAborted()
    if (!stillCurrent()) throw new Error('binding-changed')
    ...
    if (error instanceof Error && error.message === 'export-too-large') throw new Error(replyText('会话 ZIP 超过 32 MiB，未发送。请在网页 Chat 中执行 /export 下载。'))
