...
export class ImEngine {
  ...
  constructor(
    ...
    if (typeof on === 'function') {
      ...
      this.disposeEvents.push(on('api-session/removed', (...args: unknown[]) => {
        const id = args[0]
  ...
  private async inject(channel: ChannelAdapter, msg: ImMessage, sources: ImMessage[] = [msg]): Promise<void> {
    ...
    try {
      ...
      for (const media of msg.media ?? []) {
        ...
        else if (media.kind !== 'file' && media.path) content.push({ type: 'text', text: `[附件 ${media.name ?? media.kind}] ${media.path}` })
      }
