...
export class ChatCommands {
  ...
  private async idle(sessionId: string | undefined, signal: AbortSignal): Promise<void> {
    if (!sessionId) return
    if (this.pending(sessionId)) throw new Error(replyText("请先完成当前问题或审批，再切换会话。"))
    const list = await this.call<{ items: Row[] }>('sessionController', 'list', {}, signal)
    if (list.items.find(row => row.sessionId === sessionId)?.running) throw new Error(replyText("当前任务正在运行，请先 /stop，等待停止后再切换。"))
  }
  ...
  private localDetails(binding: { sessionId: string; handle?: { agent?: unknown } }): string {
  ...
  private async run(channel: ChannelAdapter, msg: ImMessage, signal: AbortSignal): Promise<string> {
    ...
    if (command === 'status' || command === 'current') {
      ...
      try {
        ...
        for await (const frame of stream) { details.push(replyText('排队消息：{0} 条', frame.value.queues[sessionId]?.length || 0)); break }
      } catch { signal.throwIfAborted() }
