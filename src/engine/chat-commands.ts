  ...
  private async run(channel: ChannelAdapter, msg: ImMessage, signal: AbortSignal, navigation: { choices?: Choice[] }): Promise<string> {
    ...
    if (command === 'export') {
      ...
      return await exportSession(this.host, channel, msg.chatId, sessionId, signal,
        () => this.router.lookup(channel.id, kind, msg.chatId)?.sessionId === sessionId)
    }
    if (command === 'help') {
    ...
    if (command === 'stop') {
      try { await this.call('sessionController', 'cancel', { sessionId }) }
      catch (error) {
        ...
        if (state?.items.find(row => row.sessionId === sessionId)?.running !== false) throw error
