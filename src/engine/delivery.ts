...
export function deliveryName(value: unknown): string {
...
export function nameKey(value: string): string { return value.trim().normalize('NFC').toLowerCase() }

export function validateDeliveryRoute(platform: ChannelId, input: Record<string, unknown>): DeliveryRoute {
  const { kind, nativeId, idType, threadId } = input
  if (kind !== 'dm' && kind !== 'group') throw new DeliveryError('invalid-target', '请选择私聊或群聊')
  if (typeof nativeId !== 'string' || !nativeId || nativeId !== nativeId.trim() || nativeId.length > 256
    || nativeId === '.' || nativeId === '..' || /[\s\u0000-\u001f\u007f/?#\\]/.test(nativeId)) throw new DeliveryError('invalid-target', '平台 ID 不合法')
  if (platform === 'weixin' && kind !== 'dm') throw new DeliveryError('invalid-target', '微信仅支持私聊投递')
  ...
  if (feishu && kind === 'group' && idType === 'open_id') throw new DeliveryError('invalid-target', '群聊必须使用 Chat ID')
  ...
  return { kind, nativeId, ...(feishu ? { idType: (idType ?? 'chat_id') as 'chat_id' | 'open_id' } : {}),
    ...(threadId === undefined ? {} : { threadId: Number(threadId) }) }
}

export function assertDeliveryText(text: unknown): asserts text is string {
...
export async function deliverText(adapter: ChannelAdapter, route: DeliveryRoute, text: string, signal?: AbortSignal): Promise<DeliveryResult> {
  assertDeliveryText(text)
  if (!adapter.sendProactive) throw new DeliveryError('unsupported', '该渠道未提供主动投递能力')
  ...
  for (const char of text) {
    ...
    if (part && (part.length + char.length > maxLength || bytes + size > 3800)) { parts.push(part); part = ''; bytes = 0 }
    part += char
    bytes += size
  }
  ...
  for (const part of parts) {
    if (signal?.aborted) return { status: receipts.length ? 'partial' : 'failed', sentParts: receipts.length, totalParts: parts.length,
