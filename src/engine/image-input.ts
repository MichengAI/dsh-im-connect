...
export function imageInputFailure(error: unknown): string {
  if (error instanceof ImageInputError) return error.message
  const reason = (error as { details?: { reason?: string } } | null)?.details?.reason
  if (reason === 'MODEL_DOES_NOT_SUPPORT_IMAGES') return '当前会话模型不支持图片输入，请在 Chat 中切换支持视觉的模型后重新发送。'
...
export async function imagePromptPart(media: ImMedia): Promise<Record<string, unknown>> {
  ...
  if (data.length === 0) throw new ImageInputError('图片下载失败或内容为空，请重新发送。')
  ...
  else if (['GIF87a', 'GIF89a'].includes(data.subarray(0, 6).toString('ascii'))) mediaType = 'image/gif'
