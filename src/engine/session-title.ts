...
export function readSessionTitle(data: unknown): { title: string; source: 'host' | 'user' } | undefined {
  if (!data || typeof data !== 'object') return undefined
  const value = data as { title?: unknown; source?: { kind?: unknown } }
  if (typeof value.title !== 'string' || !value.title.trim()) return undefined
  const kind = value.source?.kind
  if (kind !== 'user' && kind !== 'provider' && kind !== 'fallback') return undefined
