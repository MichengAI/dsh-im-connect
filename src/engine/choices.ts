...
type Entry = { key: string; session?: string; expires: number; choices: Choice[]; valid: () => boolean; allowNumber: boolean }
...
export class ChoiceStore {
  ...
  async show(channel: ChannelAdapter, msg: ImMessage, text: string, choices: Choice[], session?: string, valid: () => boolean = () => true, hint = replyText('点击选项或回复序号；15 分钟内有效，普通文字退出菜单。'), allowNumber = true): Promise<string> {
    ...
    for (const [token, entry] of this.entries) if (entry.expires <= Date.now()) { this.entries.delete(token); if (this.scopes.get(entry.key) === token) this.scopes.delete(entry.key) }
    if (this.entries.size >= 512) { const oldest = this.entries.keys().next().value!; const entry = this.entries.get(oldest)!; this.entries.delete(oldest); this.scopes.delete(entry.key) }
    ...
    if (channel.sendChoices && (!channel.choiceLimits || (choices.length <= channel.choiceLimits.maxButtons && cardBody.length <= channel.choiceLimits.maxTextLength))) {
      try { await channel.sendChoices(msg, cardBody, choices.map((choice, i) => ({ label: choice.label, token: `${token}:${i}` }))); return '' }
  ...
  resolve(channel: string, msg: ImMessage, session?: string, allowNumber = true): string | undefined {
    ...
    if (!explicit && !numeric) {
      if (!msg.text.startsWith('/')) { this.entries.delete(token); this.scopes.delete(key) }
      return undefined
    }
    if (!entry || entry.key !== key || entry.session !== session || entry.expires <= Date.now() || !entry.valid()) return ''
    ...
    this.entries.delete(token); this.scopes.delete(key)
