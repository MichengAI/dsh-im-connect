...
export class FileDelivery {
  ...
  deliver(session: DeliverySession, closing: Event, target: () => { channel: ChannelAdapter; chatId: string } | undefined): Promise<void> {
    ...
    return this.queue.run(String(session.id), async () => {
      ...
      for (const path of selected.paths) {
        if (sent.has(path) || this.lifetime.signal.aborted) continue
        const current = target()
        if (!current || current.channel !== initial.channel || current.chatId !== initial.chatId) return
        ...
        try {
          ...
          const latest = target()
          if (!latest || latest.channel !== initial.channel || latest.chatId !== initial.chatId) return
