...
export function parseWecomGenerate(body: unknown, now = Date.now()): { scode: string; verificationUrl: string; expiresAt: number } {
  const data = asRecord(asRecord(body).data)
  const scode = cleanString(data.scode)
  const verificationUrl = safeVerificationUrl(data.auth_url)
  if (!scode || !verificationUrl) throw new Error('企业微信二维码服务返回数据不完整')
  return { scode, verificationUrl, expiresAt: now + QR_TTL_MS }
}

export function parseWecomPoll(body: unknown): PairingPoll {
  const data = asRecord(asRecord(body).data)
  ...
  if (state === 'success') {
    ...
    if (!botId || !secret) throw new Error('企业微信扫码结果缺少 Bot 凭据')
    return { status: 'success', credentials: { botId, secret } }
  }
  if (state === 'expired' || state === 'timeout') return { status: 'expired', error: '二维码已过期' }
  if (state === 'fail' || state === 'failed' || state === 'error') return { status: 'failed', error: '扫码未完成' }
  return { status: 'waiting' }
}

export async function beginWecomQr(signal?: AbortSignal): Promise<PairingBegin> {
