...
export function wecomMessageId(value: unknown): string | undefined {
...
export class WecomReplyBroker {
  ...
  async sendCard(chatId: string, messageId: string | undefined, card: unknown, fullText?: string): Promise<void> {
    this.prune(chatId)
    const item = messageId ? this.pending.get(chatId)?.find(entry => wecomMessageId(frameBody(entry.frame).msgid) === messageId) : undefined
    if (fullText) {
      if (item) await this.client.replyStream(item.frame, item.streamId, fullText, true)
      else await this.client.sendMessage(chatId, { msgtype: 'markdown', markdown: { content: fullText } })
    }
    try {
