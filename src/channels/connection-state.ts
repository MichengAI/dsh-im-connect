...
export function connectionState(status: string): ConnectionState {
  if (readyStates.has(status)) return 'connected'
  if (connectingStates.has(status)) return 'connecting'
  if (status === '重连中') return 'reconnecting'
  if (status === '已停止') return 'stopped'
  if (status === '未连接' || status === '未登录' || /^已断开(?:（.*）)?$/.test(status)) return 'disconnected'
  if (status === '重连失败' || status === '连接错误' || status === '连接失败' || status === '连接失败，请查看本机日志' || status === '轮询异常（详情见本机日志）') return 'error'
  return 'unknown'
}
