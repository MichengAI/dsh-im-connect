...
function parseDingtalkCardEnvelope(raw: string): { cardId: string; userId: string; privateData: any } | undefined {
  try {
    const data = JSON.parse(raw)
    if ((data.type !== undefined && data.type !== 'actionCallback') || (data.userIdType !== undefined && data.userIdType !== 1 && data.userIdType !== '1')) return
    const content = typeof data.content === 'string' ? JSON.parse(data.content) : data.content
    if (typeof data.outTrackId !== 'string' || !data.outTrackId || typeof data.userId !== 'string' || !data.userId) return
    return { cardId: data.outTrackId, userId: data.userId, privateData: content?.cardPrivateData }
  } catch { return }
}

export function parseDingtalkCardAction(raw: string, allowed?: ReadonlySet<string>): { cardId: string; userId: string; token: string } | undefined {
  ...
  if (!allowed) return direct ? { cardId, userId, token: direct } : undefined
  ...
  const matches = [...new Set(candidates.filter((value): value is string => typeof value === 'string' && allowed.has(value)))]
