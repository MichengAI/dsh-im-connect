...
import { DeliveryError, type DeliveryRoute, type DeliveryReceipt } from '../engine/delivery.js'
...
export class DingtalkCardClient {
  ...
  async sendProactive(route: DeliveryRoute, text: string, signal?: AbortSignal): Promise<DeliveryReceipt> {
    signal?.throwIfAborted()
    let token: string
    try { token = await this.accessToken() } catch { throw new DeliveryError('authentication-failed', '钉钉账号鉴权失败，请重新连接', 503) }
    ...
    const data = await response.json() as { code?: string | number; processQueryKey?: string; invalidStaffIdList?: string[]; flowControlledStaffIdList?: string[] }
    ...
    if (!response.ok || !data.processQueryKey) throw new Error('钉钉未返回有效发送回执')
